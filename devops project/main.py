...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allows all origins for deployment. For production, specify your Vercel URL.
...
def callback(code: str, state: str = None):
    try:
        return _do_mastodon_callback(code)
    except Exception as exc:
        import traceback
        traceback.print_exc()
        # Redirect to frontend error page so user sees what went wrong
        error_msg = urllib.parse.quote(str(exc)[:200])
        return RedirectResponse(
            url=f"{settings.FRONTEND_URL}/#/result?error={error_msg}",
            status_code=302
        )

def _do_mastodon_callback(code: str):
    ...
    # Direct redirect to frontend — use hash routing (#/analyze) to match the SPA
    # Use 302 (not 307) so browsers always follow as GET
    return RedirectResponse(url=f"{settings.FRONTEND_URL}/#/analyze?id={result_id}", status_code=302)
