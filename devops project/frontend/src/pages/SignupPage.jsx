    ...
    <div className="page">
      ...
      <main className="page-content" style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '128px 24px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.4em', color: 'var(--ember)' }}>Account</div>
          <h1 style={{ marginTop: 12, fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem, 8vw, 4rem)', lineHeight: 1 }}>Sign Up</h1>
          <p style={{ marginTop: 16, color: 'var(--muted-foreground)' }}>Create an account to submit posts for rental risk analysis.</p>
        </div>

        <form onSubmit={handleSignup} className="glass" style={{ borderRadius: 20, padding: 32 }}>
          <div style={{ marginBottom: 16 }}>
            <label className="label">Display Name</label>
            <input className="input" type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Your name" required />
          </div>
          <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ marginTop: 24, textAlign: 'center', fontSize: 14, color: 'var(--muted-foreground)' }}>
            Already have an account? <a href="#/login" style={{ color: 'var(--ember)', textDecoration: 'none' }}>Log in</a>
          </p>
        </form>
      </main>
    </div>
